...
export default function EditIdeaPage() {
    ...
    async function fetchIdea() {
      try {
        ...
        setFormData({
          ...
          price_hint: idea.price_hint || '',
        })
        setSlug(idea.slug || '')
        setOriginalSlug(idea.slug || '')
      } catch (err) {
  ...
  // Save changes
  const saveChanges = async (newStatus?: 'draft' | 'live') => {
    ...
    try {
      ...
      if (result.slug_changed) {
        alert(`⚠️ Slug changed from "${result.old_slug}" to "${slug}".\n\nOld links will no longer work. Consider setting up a redirect or updating any shared links.`)
      }
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <p className="mt-2 text-gray-600">
            Update your idea details. Changes are reflected immediately on your landing page.
          </p>
          {slug !== originalSlug && (
            <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
              <p className="text-sm text-yellow-800">
                ⚠️ You've changed the URL slug. Old links will break unless you set up a redirect.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-lg shadow p-8">
          ...
          {step === 'preview' && (
            <div className="space-y-6">
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <h3 className="font-semibold text-blue-900 mb-2">📝 Save Your Changes</h3>
                <p className="text-sm text-blue-800">
                  Your changes will be reflected on your landing page immediately.
                </p>
              </div>
            </div>
          )}
